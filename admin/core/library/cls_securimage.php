...
{
    public $text_num; //字符个数
    public $interference_line; //干扰线:ture、false
    public $ttf_file; //字体
    public $im_x;
    public $im_y;
    public $session_name; //session名称

    function  __construct($text_num=4, $im_x = 110, $im_y = 30, $scale = 5, $session_name='securimage_code_value')
    {
        $this->text_num = $text_num;
        $this->interference_line = true;
        $this->ttf_file = PATH_SHARE.'/securimage_font/'.rand(4,6).'.ttf';//随机选6种字体
        $this->im_x = $im_x;
        $this->im_y = $im_y;
        $this->scale = $scale;
    ...
    function show()
    {
        ...
        $this->make_rand(); //生成text_num个随机字符,并赋值给session
    ...
    /*
     * 生成text_num个随机字符,并赋值给session
     * */
    function make_rand()
    {
        $str="ABCDEFGHJKMNPQRSTUVWXYZabcdefghkmnprstuvwxz23457";
        $result="";
    ...
    function check($code)
    {
        ...
        {
            //验证成功一次就清空一下session，防止重复提交同一个验证码进行攻击
            //当用户得到验证码后，只要不从新访问页面，验证码是不会重新生成的，这里必须清空，让他必须再到页面去获得验证码
